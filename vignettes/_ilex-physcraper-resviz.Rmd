...
An easy way to visualize the new tips added into the updated tree compared to the original tree is coloring the tips and/or branches from those new tips with a vibrant color. We will use a function defined in this package to do so.
...
First plot just the new tips

Then plot the tips and branches

```{r plot-tips-and-branches}
plot_branches(treefile = mytreefile, otufile = myotuinfofile, type = "phylogram", cex = 0.3, edge.width = 1, tip_label = "taxon")
```
